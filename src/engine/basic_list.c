    ...
    const struct memory_allocation_key *outNodeKeyPtr)
{
    ...
    if (lhsListPtr->freeListNodeCount < 1)
    {
        memory_unmap_alloc((void **)lhsListPtr);
        memory_unmap_alloc((void **)rhsListPtr);
    ...
    if ((_basic_list_dettach_next_free_node(lhsListKeyPtr, &nodeKey)))
    {
        ...
        if (resultCode != MEMORY_OK)
        {
            ...
            basic_list_free_node(lhsListKeyPtr, &nodeKey);
...
basic_list_destroy(const struct memory_allocation_key *listKeyPtr)
{
    ...
    if (!(basic_list_get_prev_node(listKeyPtr, &listPtr->activeNodeKeyList, &tailNodeKey)))
    {
        memory_unmap_alloc((void **)&listPtr);
    ...
    do 
    {
        ...
        if (!(basic_list_get_prev_node(listKeyPtr, &currentNodeKey, &nextNodeKey)))
        {
            memory_unmap_alloc((void **)listPtr);
