           ...
           utils_sort_replace_func replaceFunc, void *userPtr)
{
    ...
    for (u8 *iter = begin + stride; iter != end; iter += stride)
    {
        ...
        void *j = (u8 *)element - stride;
        size_t jdex = elementIndex - 1;

        for (; j >= location; (u8 *)j -= stride, --jdex)
        {
            replaceFunc(mem, (u8 *)j + stride, jdex + 1, j, jdex, stride, userPtr);
        }

        replaceFunc(mem, (u8 *)j + stride, jdex + 1, elementCopy, elementIndex, stride, userPtr);
    }
